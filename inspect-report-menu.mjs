...
await page.screenshot({ path: "salesforce-report-menu.png", fullPage: true });
const items = await page.locator("a, button, span").evaluateAll((nodes) => nodes.map((node) => ({
  text: (node.innerText || node.textContent || "").trim(),
  title: node.getAttribute("title"),
  aria: node.getAttribute("aria-label"),
  role: node.getAttribute("role"),
  tag: node.tagName,
})).filter((item) => `${item.text} ${item.title} ${item.aria}`.match(/export|save|delete|subscribe|print|run|edit|clone|more/i)));
